...
while(~isempty(outstanding_nodes))
  ...
  % gf = @(v) gain_function(X, new_clique, v, ct_control);
   %[nodes gains seps] = arrayfun(gf, outstanding_nodes.', 'UniformOutput', false);
   ...
   GT.separators(idx, :) = seps;
   GT.nodes(idx)= nodes;
   % remove from gain table where clique is the old one and node is the new
   % one
   idx = find(GT.nodes == the_node);
   GT.gains(idx) = NaN;
   %idx = id_from_set(GT.cliques, parent_clique);
   %GT.gains(idx) = NaN;
